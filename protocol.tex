...
\end{align*}
Thus $F(y)$ has no extrema, if $\Gamma A\geq1/3$. Through the choice of the parameters $\sqrt{2}\,\Gamma/\kappa$ the position and existence of the roots can be controlled. But it has to be remembered that these parameters only can be positive. The effect of this is examined by resolving the above equation for $\Gamma A$, and inserting it into $F$.
\begin{align*}
    (3\,y_{12}-2)^2=&1-3\,\Gamma A\\
    \Gamma A=& \frac{1}{3}\,\left(  1-(3\,y_{12}-2)^2 \right)\\\\
    \Rightarrow\quad F(y_{12})=&-\sqrt{2}\,\Gamma/\kappa-\frac{1}{3}\,\left(  1-(3\,y_{12}-2)^2 +3\right)\,y_{12}-y_{12}^3+2\,y_{12}^2\\
    =&-\sqrt{2}\,\Gamma/\kappa-\frac{1}{3}\,\left( - 9\,y_{12}^2+12\,y_{12}\right)\,y_{12}-y_{12}^3+2\,y_{12}^2\\
    =&-\sqrt{2}\,\Gamma/\kappa+2\,y_{12}^3-2\,y_{12}^2\\
\end{align*}
...
\end{align*}
So $\Gamma A$ can be minimally $-1/8$. For a certain value of $A$ the corresponding $\kappa$ would be
\begin{align*}
...
\begin{wrapfigure}[18]{l}{0pt}
    \includegraphics{k=w_F|B_numn_of_fixp.pdf}
    \vspace*{-2cm}\caption{The cource of $F+B$ and $B$ as function of $\Gamma A$}
    \label{fig:k=w_numb_of_fixp}
\end{wrapfigure}\\
The minimal value of $B$ is taken again in the case $A<0$ for the minus sign and also again here for $\gamma=0$.\\
...
\begin{wrapfigure}[13]{l}{0pt}
    \includegraphics{k=w_sign_of_ev1.pdf}
...
\end{align*}
The expression inside the square root is maximized by minimizing $A$ with respect to $\kappa$ and $\gamma$, what results in $\Gamma A = -1/2$, which can be inferred by the previous calculation. As a consequence the expression in the square root is always negative, which means that p is always positive. For the proceeding examination we keep in mind that the eigenvalues are determined by
\begin{align*}
